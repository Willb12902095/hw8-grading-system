...
def detect_student_folders(base_path: Path) -> List[Path]:
    """
    Detect student submissions.

    Supported formats:
    1. Flat files named student#_file.py
    2. Student folders containing code files and assets

    Non-code asset files at the top level are ignored so images/sounds are not
    treated as fake students.
    """
    student_folders = {}
    skipped_files = []
    result = []
    ...
    def is_junk_path(path: Path) -> bool:
        ...
        return (
            ...
            path.name == '.DS_Store' or
            '__MACOSX' in parts or
            path.name == '__MACOSX'
        )

    def is_code_file(path: Path) -> bool:
        return path.is_file() and not is_junk_path(path) and path.suffix.lower() in REQUIRED_CODE_EXTENSIONS

    def is_chat_file(path: Path) -> bool:
        return (
            path.is_file() and
            not is_junk_path(path) and
            path.suffix.lower() == '.txt' and
            path.name.lower() in ['chat.txt', 'conversation.txt', 'history.txt']
        )
    ...
    for item in base_path.iterdir():
        ...
        if item.is_dir():
            files = [f for f in item.rglob('*') if f.is_file() and not is_junk_path(f)]
        ...
        if item.is_file():
            ...
            # Skip nested archive files that were not extracted.
            if item.suffix.lower() in ['.7z', '.rar', '.zip']:
                skipped_files.append(item)
                continue

            # Ignore assets and other non-code/non-chat files at the top level.
            # They may be needed during runtime, but they should not become students.
            if not is_code_file(item) and not is_chat_file(item):
                continue
    ...
    if skipped_files:
        print(f"  ⚠ Skipped {len(skipped_files)} archive file(s):")
        for sf in skipped_files:
            print(f"    - {sf.name}")

    for student_name, files in sorted(student_folders.items()):
        # Only keep entries that actually contain code files
        code_files = [f for f in files if is_code_file(f)]
        if not code_files:
            continue
...
def find_code_files(folder: Path, file_list: List[Path] = None) -> List[Path]:
    ...
    if file_list:
        ...
        for f in file_list:
            if f.is_file() and f.suffix.lower() in REQUIRED_CODE_EXTENSIONS and not f.name.startswith('._') and '__MACOSX' not in f.parts:
                code_files.append(f)
    else:
...
def find_chat_history(folder: Path, file_list: List[Path] = None) -> List[Path]:
    ...
    if file_list:
        # Use provided file list (for file-based detection)
        for f in file_list:
            if f.is_file() and not f.name.startswith('._') and '__MACOSX' not in f.parts:
                if f.name.lower() in ['chat.txt', 'conversation.txt', 'history.txt'] and f.suffix.lower() == '.txt':
                    chat_files.append(f)
    else:
        # Use folder search (for backward compatibility)
...
def create_grading_log(grades: List[Dict], output_path: str):
    ...
    with open(output_path, 'w') as f:
        ...
        # Add rubric info
        if use_code_only:
            max_total = sum(item['max_points'] for item in CODE_ONLY_RUBRIC.values())
            f.write(f"Rubric: CODE-ONLY (max score = {max_total}; percentage used for grade)\n")
        else:
